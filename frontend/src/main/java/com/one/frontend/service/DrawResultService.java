	...
	public List<DrawResult> handleDraw2(Long userId, Long productId, List<String> prizeNumbers) throws Exception {
		try {
			...
			PrizeNumber selectedPrizeNumber = availablePrizeNumbers.stream()
					.filter(pn -> prizeNumbers.contains(pn.getNumber())).findFirst()
